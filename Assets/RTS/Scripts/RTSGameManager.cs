...
namespace es.ucm.fdi.iav.rts
{
    ...
    public class RTSGameManager : MonoBehaviour
    {
        ...
        [SerializeField] private GameObject GrabenCamp;
        ...
        // Las torretas neutrales, que no tienen asignado un controlador y por tanto un índice
        private List<Tower> NeutralTowers { get; set; } = new List<Tower>();

        int width, height;
        Vector3 terrainPos;

        public GameObject terrain;
        RTSWorldToMap conversor;
        ...
        private void Awake()
        {
            ...
            else
            {
                ...
                NeutralTowers = new List<Tower>(GrabenCamp.GetComponentsInChildren<Tower>());
            }
            Vector3 size = terrain.GetComponent<Collider>().bounds.size;
            width = (int)Math.Round(size.z * terrain.transform.localScale.z);
            height = (int)Math.Round(size.x * terrain.transform.localScale.x);
            terrainPos = terrain.transform.position;
            conversor=this.gameObject.AddComponent<RTSWorldToMap>();
        }
        ...
        // Devuelve las torretas neutrales que hay en el escenario. Como no tienen un controlador, no hay un índice asignado a su bando.
        public List<Tower> GetTurrets()
        {
            return NeutralTowers;
        }
        ...
        public int getWidth() { return width / cellSize; }
        public int getHeight() { return height / cellSize; }
        public Vector3 getTerrainPos() { return terrainPos; }
        public List<RTSWorldToMap.Info>[,] getForceMap() { return conversor.getForceMap(); }
        public List<RTSWorldToMap.Info>[,] getValueMap() { return conversor.getValueMap(); }
    }
}